...
sed -i "s/SIDE_2_IP/${LINK_DEV_IP}/g" ./case.yaml
sed -i "s/SIDE_1_IP/${HOST_IP}/g" ./case.yaml
...
# Start Python Side
cd ./src
docker run --rm -d -v `pwd`:`pwd` -w `pwd` --net=mymacvlan --ip=$LINK_DEV_IP --name python_side grpc-python3 python3 sanity_side.py 0.0.0.0 ${GRPC_PORT}
