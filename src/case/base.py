...
class TestCase:
    ...
    def run(self, test_name):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            future2side = {}
            if side1_cmd:
                tmp_future = executor.submit(process_command, side1_cmd, self.side1,
                                             info1, self.stub1, self.side2, info2, self.stub2)
                future2side[tmp_future] = 'side_1'
                cmd_future.append(tmp_future)
            if side2_cmd:
                tmp_future = executor.submit(process_command, side2_cmd, self.side2,
                                             info2, self.stub2, self.side1, info1, self.stub1)
...
def connect_qp(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    timeout = c_arg.get('timeout', 14)
    retry = c_arg.get('retry', 7)
    rnr_retry = c_arg.get('rnr_retry', 7)
    self_stub.ConnectQp(message_pb2.ConnectQpRequest(
        dev_name=self_info.dev_name, qp_id=self_info.qp_id, access_flag=15, gid_idx=self_side.gid_idx(), ib_port_num=self_side.ib_port(), remote_qp_num=other_info.qp_num, remote_lid=other_info.lid, remote_gid=other_info.gid, timeout=timeout, retry=retry, rnr_retry=rnr_retry))
    return True


def sleep(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    len = c_arg.get('len')
    time.sleep(len)
    return True


def recv_pkt(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    retry = c_arg.get('wait_for_retry', False)
    self_stub.RecvPkt(message_pb2.RecvPktRequest(
        wait_for_retry=retry, has_cqe=True, qp_id=self_info.qp_id))
    return True


def local_check(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    ...
    resp = self_stub.LocalCheckMem(message_pb2.LocalCheckMemRequest(
...
def local_write(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    offset = c_arg.get('offset', 0)
    ...
    self_stub.LocalWrite(message_pb2.LocalWriteRequest(
        mr_id=self_info.mr_id, offset=offset, len=len(content), content=content))
    return True


def remote_read(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    local_offset = c_arg.get('local_offset', 0)
    remote_offset = c_arg.get('remote_offset', 0)
    len = c_arg.get('len', 0)

    self_stub.RemoteRead(message_pb2.RemoteReadRequest(addr=(self_info.addr + local_offset), len=len, lkey=self_info.lkey,
                                                       remote_addr=(other_info.addr + remote_offset), remote_key=other_info.rkey, qp_id=self_info.qp_id, cq_id=self_info.cq_id))
    return True


def remote_write(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    local_offset = c_arg.get('local_offset', 0)
    remote_offset = c_arg.get('remote_offset', 0)
    len = c_arg.get('len', 0)

    self_stub.RemoteWrite(message_pb2.RemoteWriteRequest(addr=(self_info.addr + local_offset), len=len, lkey=self_info.lkey,
                                                         remote_addr=(other_info.addr + remote_offset), remote_key=other_info.rkey, qp_id=self_info.qp_id, cq_id=self_info.cq_id))
    return True


def remote_send(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    offset = c_arg.get('offset', 0)
    len = c_arg.get('len', 0)

    self_stub.RemoteSend(message_pb2.RemoteSendRequest(addr=(self_info.addr + offset), len=len, lkey=self_info.lkey,
                                                       qp_id=self_info.qp_id, cq_id=self_info.cq_id))
    return True


def local_recv(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    offset = c_arg.get('offset', 0)
    len = c_arg.get('len', 0)

    self_stub.LocalRecv(message_pb2.LocalRecvRequest(addr=(self_info.addr + offset), len=len,
                                                     lkey=self_info.lkey, qp_id=self_info.qp_id, cq_id=self_info.cq_id))
    return True


def unblock_other(c_arg, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
...
def process_command(cmds, self_side: Side, self_info: SideInfo, self_stub: SideStub, other_side: Side, other_info: SideInfo, other_stub: SideStub):
    for c in cmds:
        if not c['name']:
        ...
        if fun:
            try:
                if not fun(c, self_side, self_info, self_stub, other_side, other_info, other_stub):
