...
print(f'received client metadata: {parsed_fields}')
...
roce.recv_pkts(2) # Receive two send requests both with wrong PSN, the second one should be discarded
cqe = qp.poll_cq()
...
qp.post_send(sr)
qp.process_one_sr()
roce.recv_pkts(1) # Receive the ACK to the last write operation, implicit NAK to previous atomic operation

cqe = qp.poll_cq() # This CQE is for previous send operation
assert cqe is not None, 'send without signaled should have CQE in responder after implicit ACK'
assert cqe.local_qpn() == qp.qpn()
assert cqe.sqpn() == dst_qpn
...
qp.post_send(sr)
qp.process_one_sr()
sleep_time_sec = (Util.timeout_to_ns(S_TIMEOUT) * 2) / 1_000_000_000
print(f'sleep {sleep_time_sec} seconds')
time.sleep(sleep_time_sec) # Wait to be timeout
